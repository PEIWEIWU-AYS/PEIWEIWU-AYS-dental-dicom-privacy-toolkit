...
from ddpt.showcase import build_showcase_report
...
showcase_app = typer.Typer(help="Build public synthetic showcase galleries.")
...
def showcase_build(
    ...
    output_path: Annotated[
        Path | None,
        typer.Option("--out", help="Output showcase HTML path."),
    ] = None,
    json_output: Annotated[
        Path | None, typer.Option("--json", help="Write JSON showcase report.")
    ] = None,
) -> None:
    output_path = output_path or evidence_dir / "reports" / "showcase.html"
    json_output = json_output or output_path.with_suffix(".json")
    try:
        report = build_showcase_report(evidence_dir, output_path)
    except FileNotFoundError as exc:
        console.print(str(exc))
        raise typer.Exit(1) from exc
    write_json(json_output, model_to_dict(report))
    write_showcase_html(output_path, report)

    table = Table(title="Dental DICOM Showcase Gallery")
    table.add_column("Status")
    table.add_column("Category")
    ...
    console.print(table)
    console.print(f"Showcase HTML: {output_path}")
    console.print(f"Showcase JSON: {json_output}")
    console.print(f"Reports: {report.available_items}/{report.total_items}")
    console.print(f"Previews: {report.available_previews}/{report.total_previews}")
    console.print(f"Overall: {'PASS' if report.passed else 'FAIL'}")
    if not report.passed:
        raise typer.Exit(1)
